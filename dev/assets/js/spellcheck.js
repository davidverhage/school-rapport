...
;(function (window, document, undefined) {
    ...
    sc.SpellChecker = function (options) {
        ...
        this._settings = {
            ...
            debug: false,
            onOpen: function (button, text) {
            },    // Callback to be executed when spell checker is opened
            onClose: function (button, text) {
            }    // Callback to be executed after spell checker is closed
        };
    ...
    sc.SpellChecker.prototype = {
        ...
        _notifyMsg: function (type) {
            ...
            if (type == 'b') {
                msg = 'Er is een fout opgetreden voor het afronden van controle...';
            }

            if (type == 'c') {
                msg = 'Gereed. Er zijn geen fouten gevonden.';
            }

            if (type == 'd') {
                msg = 'Spellingcontrole uitgevoerd.';
            }
        ...
        _createHTML: function () {
            ...
            this._modal.innerHTML = '<div class="spell-header"><div>Spellingscontrole</div></div><div id="spelling-inner' + this._uId + '" class="spelling-inner"><div class="clearleft">Niet in bibliotheek gevonden:</div><div class="spell-nf"><input type="text" class="current" id="spell-current' + this._uId + '" /><div class="context" contenteditable="true" id="spell-context' + this._uId + '"></div></div><div class="spell-ignorebtns"><button id="spelling-ignore' + this._uId + '" type="button">Overslaan</button><button id="spelling-ignore-all' + this._uId + '" type="button">Alles Overslaan</button></div><div class="clearleft">Suggesties:</div><div class="spell-suggest"><select size="8" id="spelling-suggestions' + this._uId + '"><option></option></select></div><div class="spell-changebtns"><button type="button" id="spell-change' + this._uId + '">Wijzig</button><button id="spell-change-all' + this._uId + '">Alles wijzigen</button><button type="button" id="spell-undo' + this._uId + '">Ongedaan maken</button></div><hr /><div class="close-box"><button type="button" id="spell-close' + this._uId + '">Sluiten</button></div></div>';
            document.body.appendChild(this._modal);

            // Popup message box
            this._msgBox.className = 'spell-msg';
            this._msgBox.innerHTML = '<div class="spell-header"><div>Spellingscontrole</div></div><div class="spell-msg-inner"><span id="spell-msg-text' + this._uId + '"></span></div><div class="spell-msg-inner" id="spell-msg-close-box' + this._uId + '"><button id="spell-msg-close' + this._uId + '">OK</button></div>';
