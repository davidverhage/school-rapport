...
class Peuter extends App\MY_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->user->has_permission('leidsters')) {
            ...
            $content = $this->load->view('peuter/dashboard_view', $data, true);
        }
    ...
    function edit($id = null, $version = '1', $schooljaar = '2016-2017', $group = '1')
    {
        $result = $this->student_model->get_student_report($id, $group, $version, $schooljaar, $group);

        if ($result->num_rows() >= 1) {
            $data['students'] = $result;
            if ($group === 0 || $group === '0') {
                $content = $this->load->view('peuter/verslagps', $data, true);
            } else {
                $content = $this->load->view('peuter/verslag', $data, true);
            }
            $this->render($content);
        } else {
            $data = ['user_id' => $this->user->get_id(), 'tbl_student_id' => $id, 'verslag_versie' => $version, 'schooljaar' => $schooljaar];
            $db_group = 'tbl_g' . $group;
            if ($this->student_model->insert_student_report($db_group, $data)) {
                $page = base_url("peuter/edit/{$id}/{$version}/{$schooljaar}/{$group}");
    ...
    function create($id = null)
    {
        ...
        $content = $this->load->view('peuter/index', $data, true);
        $this->render($content);
    }
    ...
    function delete($id = null)
    {
        if ($id == null) {
            ...
            $content = $this->load->view('peuter/index', null, true);
            $this->render($content);
        }

    }

    function store($id, $group, $schooljaar, $student_id)
    {
    ...
    function multipdf()
    {
        ...
        $content = $this->load->view('peuter/view_files', $data, true);
        $this->render($content);
    ...
    public function importexcel()
    {
        ...
        if (!$this->upload->do_upload()) {
            ...
            $file_path = $uploaddir . $file_data['file_name'];
