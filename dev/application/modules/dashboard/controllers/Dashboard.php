...
class Dashboard extends App\MY_Controller
{
    ...
    function edit($id = null, $version = '1', $schooljaar = '2016-2017', $group = '1')
    {
        ...
        if ($result->num_rows() >= 1) {
            ...
            if ($this->student_model->insert_student_report($db_group, $data)) {
                $page = base_url("dashboard/edit/{$id}/{$version}/{$schooljaar}/{$group}");
    ...
    function store_pdf($student_id, $lhg, $report_id, $schooljaar = "2017-2018", $download = false)
    {
        $filename = '';
        $directory = 'doc/';
        ...
        */
        ...
        $this->db->where('verslag_versie', 2);
        $this->db->where('schooljaar', $schooljaar);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $data['enabled'] = true;
            $uri_table = $report_table;
