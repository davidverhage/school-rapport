...
class User_manager
{
    ...
    function update_user($full_name, $login, $email, $password, $active = 1, $user_id)
    {
        $hashed_password = $this->CI->bcrypt->hash($password);
        return $this->CI->db->update('users', array('name' => $full_name, 'login' => $login, 'email' => $email, 'password' => $hashed_password, 'active' => $active), array('id' => $user_id));
    }
    ...
    function update_permission($permissions = array(), $user_id)
    {
        $this->CI->db->update('users_permissions', array('permission_id' => $permissions), array('id' => $user_id));
    }
